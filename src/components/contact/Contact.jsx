...
const Contact = ({contactRef}) => {
    ...
    const colorStyles = {
        ...
        option: (styles, state) => {
            return{
                ...styles,
                padding: '15px 20px',
                backgroundColor: state.isSelected ? 'rgb(191,219,254)' : '#ffff',
    ...
    return(
        ...
        <section class="bg-darkest px-20">
            <div class="py-8 md:pt-40 md:pb-20 w-[700px] m-auto">
                <h2 class="mb-10 text-4xl tracking-tight font-bold md:text-center text-left text-fancy ">Contact Me</h2>
                <form action="#" class="space-y-8">
                        <div className="grid grid-cols-2 gap-10">
                            <div className="group">
                                {/* <label for="name" class="block mb-2 group-focus-within:font-normal font-light text-fancy">First name</label> */}
                                <input  onChange={handleName} type="text" id="name" class={`${name.length > 0 ? 'bg-darker caret-white' : 'bg-darker'} block p-4 w-full placeholder-gray-300 focus:placeholder-white text-text-primary group focus:bg-darkest rounded-lg outline-none border-4 border-transparent focus:border-fancy font-light text-lg`} placeholder="Your first name" required />
                            </div>
                            <div className="group">
                                ...
                                <input onChange={handleLastName} type="text" id="lastname" class={`${lastName.length > 0 ? 'bg-darker caret-white' : 'bg-darker'} block p-4 w-full placeholder-gray-300 focus:placeholder-white text-text-primary group focus:bg-darkest rounded-lg outline-none border-4 border-transparent focus:border-fancy font-light text-lg`} placeholder="Your last name" required />
                            </div>
                        </div>
                        <div className="group">
                            ...
                            <input onChange={handleEmail} type="text" id="email" class={`${email.length > 0 ? 'bg-darker caret-white' : 'bg-darker'} block p-4 w-full placeholder-gray-300 focus:placeholder-white text-text-primary group focus:bg-darkest rounded-lg outline-none border-4 border-transparent focus:border-fancy font-light text-lg`} placeholder="Your email" required />
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea id="message" rows="8" class="placeholder-gray-300 focus:placeholder-white block p-4 w-full text-text-primary min-h-[200px] max-h-[350px] bg-darker rounded-lg border-4 border-transparent focus:border-fancy focus:bg-darkest outline-none font-light text-lg" placeholder="Tell me about your idea..."></textarea>
                        </div>
                        ...
                        <a href="#" className=" sm:max-w-max w-full mt-6 block rounded-md text-center 
